...
public class TrollGiant : MonoBehaviour
{
    [Header("Etc")]
    ...
    {
        Debug.Log($"Monster - Attack : {other.gameObject.tag}");
    }
    public    void OnAttacked(Collider other)
    {
        Debug.Log($"Monster - Attacked : {other.gameObject.tag}");
        agent.Move(-transform.forward); // 넉백
    }
    // events
    private void OnTriggerEnter(Collider other) {
        if(player.attackScript.IsAttacking)
            OnAttacked(other);
    }
}
